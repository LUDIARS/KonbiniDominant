...
## 2026-09-09: Phase 1 単独の実装範囲

今回の実装は [phase-1-game-loop.md](phase-1-game-loop.md) を適用する。
囲み破壊は3秒の連続包囲で自動発火、重複強化は Max、支配は現在人口の60%。
境界上の点も三角形内とする。終了後は Phase 2 ではなく Result に入る。
以下の Phase 2 進行記述と未決表示は将来の全体版の記録として残す。
